...
<head>
    ...
    <title>Category | Four Seasons</title>
    <!-- Datatable -->
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example3" class="display min-w850">
                                        ...
                                        <tbody>
                                        <?php
                                        $category_data = $db_handle->runQuery("SELECT category.c_name, product.p_name, stock.quantity, stock.inserted_at FROM `stock`,`product`,`category` WHERE stock.category_id = category.id AND stock.product_id = product.id");
                                        $row_count = $db_handle->numRows("SELECT category.c_name, product.p_name, stock.quantity, stock.inserted_at FROM `stock`,`product`,`category` WHERE stock.category_id = category.id AND stock.product_id = product.id");
                                        ...
                                        for ($i = 0; $i < $row_count; $i++) {
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo $category_data[$i]["p_name"]; ?></td>
                                                <td><?php echo $category_data[$i]["quantity"]; ?></td>
                                                <?php
