...
if (isset($_GET['subcatId'])) {
        $data = $db_handle->runQuery("select * FROM `sub_category` WHERE sub_cat_id ='{$_GET['subcatId']}'");
        unlink($data[0]['sub_cat_image']);
        $db_handle->insertQuery("delete from sub_category where sub_cat_id =" . $_GET['subcatId'] . "");
        echo 'success';
}
